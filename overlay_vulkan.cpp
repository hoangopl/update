// overlay_vulkan.cpp // Android 10 - Vulkan full-screen red overlay for ~5s // NOTE: platform-specific — may need minor adjustments per device/NDK headers. // Build: clang++ overlay_vulkan.cpp -o overlay_vulkan -lvulkan -llog -landroid

#include <unistd.h> #include <stdio.h> #include <stdlib.h> #include <string.h> #include <errno.h>

#include <vulkan/vulkan.h> #include <vulkan/vulkan_android.h>

#include <gui/SurfaceComposerClient.h> #include <gui/SurfaceControl.h> #include <gui/Surface.h> #include <ui/DisplayInfo.h> #include <utils/StrongPointer.h>

using namespace android;

static void fail(const char* msg) { fprintf(stderr, "%s\n", msg); exit(1); }

int main(int argc, char** argv) { // 1) Create a SurfaceControl (same as your previous code) sp<SurfaceComposerClient> client = new SurfaceComposerClient(); if (client->initCheck() != NO_ERROR) fail("SurfaceComposerClient init failed");
