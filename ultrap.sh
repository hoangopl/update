...
for fake in frida_server gum_js_loop libfrida_agent_so frida_trace spawn_payload interceptor rpc_exports;do
export "$fake"="this_is_fake_data"
done
check_maps_for_frida(){
  shopt -s nullglob
  local files=(/proc/*/maps)
  shopt -u nullglob
  for mapfile in "${files[@]}"; do
    grep -iq "frida" "$mapfile" && return 0
  done
  return 1
}
command -v netstat >/dev/null&&HAS_NETSTAT=1||HAS_NETSTAT=0
command -v ss >/dev/null&&HAS_SS=1||HAS_SS=0
if pgrep -fE "frida|gum-js-loop" >/dev/null||\
ps -A 2>/dev/null|grep -i "frida" >/dev/null||\
[ -d "/data/data/com.frida.server" ]||\
check_maps_for_frida||\
(command -v lsof >/dev/null&&lsof 2>/dev/null|grep -iq "frida")||\
...
for i in $(seq 1 5); do    
    (head -c 1M < /dev/urandom > /dev/null &) >/dev/null 2>&1    
done   
...
while true; do
    ...
    read ff_version
    if [[ "$ff_version" == "32" ]]; then
        echo -e "${YELLOW}Bạn đã chọn phiên bản 32-bit${RESET}"
        echo -e "${BLUE}Chạy Free Fire 32-bit...${RESET}"
GAME_PKG="com.dts.freefireth"
APP_DIR="/data/app/$(ls /data/app | grep $GAME_PKG)"
LIB_DIR="$APP_DIR/lib/arm"
OAT_DIR="$APP_DIR/oat/arm"
BASE_DM="$APP_DIR/base.dm"
chmod_000_libs=(
  "libcrashlytics.so" "libcrashlytics-common.so" "libcrashlytics-handler.so" "libcrashlytics-trampoline.so"
  "libCustomVideoPlayer.so" "libff_voice_engine.so" "libffutil.so" "libFFWebRequest.so"
  "libFirebaseCppAnalytics.so" "libFirebaseCppApp-11_8_1.so" "libFirebaseCppCrashlytics.so"
  "libFirebaseCppMessaging.so" "libfreetype.so" "libharfbuzz.so" "libmp3lame.so" "libopus.so"
)
chmod_004_libs=(
  "libanogs.so" "libanort.so" "libGGP.so" "libil2cpp.so" "libmain.so" "libunity.so"
)
chmod_000_oat=(
  "base.odex" "base.vdex" "base.art"
)
chmod_000_base=(
  "base.dm" "base.digests"
)
for lib in "${chmod_000_libs[@]}"; do
  path="$LIB_DIR/$lib"
  [ -f "$path" ] && chmod 000 "$path"
done
for lib in "${chmod_004_libs[@]}"; do
  path="$LIB_DIR/$lib"
  [ -f "$path" ] && chmod 004 "$path"
done
for file in "${chmod_000_oat[@]}"; do
  path="$OAT_DIR/$file"
  [ -f "$path" ] && chmod 000 "$path"
done
for file in "${chmod_000_base[@]}"; do
  path="$APP_DIR/$file"
  [ -f "$path" ] && chmod 000 "$path"
done
        break
   elif [[ "$ff_version" == "64" ]]; then
        echo -e "${YELLOW}Bạn đã chọn phiên bản 64-bit${RESET}"
        echo -e "${BLUE}Chạy Free Fire 64-bit...${RESET}"
GAME_PKG="com.dts.freefireth"
APP_DIR="/data/app/$(ls /data/app | grep $GAME_PKG)"
...
"202.81.119.8" "202.81.99.8" "202.81.99.6" "202.81.99.17" "202.81.99.12" "202.81.99.15" "202.81.99.4"
"202.81.99.20" "202.81.97.163" "202.81.99.14" "202.81.199.8" "202.81.199.12" "202.81.119.5" "202.81.97.166"
)
for ip in "${blocked_ips[@]}"; do
    iptables -A OUTPUT -p tcp -d "$ip" -m connlimit --connlimit-above 1 -j DROP
done
...
pm disable com.dts.freefireth/com.beetalk.sdk.GarenaAuthActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.beetalk.sdk.plugin.GGPluginActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.facebook.CustomTabActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.facebook.CustomTabMainActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.facebook.FacebookActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.garena.pay.android.GGPayActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.garena.unity.webview.UnityWebViewActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.gms.auth.api.signin.internal.SignInHubActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.gms.common.api.GoogleApiActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.gms.games.internal.v2.resolution.GamesResolutionActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.play.core.common.PlayCoreDialogWrapperActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.linecorp.linesdk.auth.internal.LineAuthenticationActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.linecorp.linesdk.auth.internal.LineAuthenticationCallbackActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.twitter.sdk.android.core.identity.OAuthActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.unity3d.plugin.downloader.UnityDownloaderActivity > /dev/null 2>&1
pm disable com.dts.freefireth/com.vk.sdk.VKServiceActivity > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.room.MultiInstanceInvalidationService > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.background.systemalarm.SystemAlarmService > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.background.systemjob.SystemJobService > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.foreground.SystemForegroundService > /dev/null 2>&1
pm disable com.dts.freefireth/com.ano.gshell.GP6Service > /dev/null 2>&1
pm disable com.dts.freefireth/com.beetalk.sdk.account.AccountAuthenticatorService > /dev/null 2>&1
pm disable com.dts.freefireth/com.dts.freefireth.FFFirebaseMessagingService > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.datatransport.runtime.backends.TransportBackendDiscovery > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.datatransport.runtime.scheduling.jobscheduling.JobInfoSchedulerService > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.gms.auth.api.signin.RevocationBoundService > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.gms.measurement.AppMeasurementJobService > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.gms.measurement.AppMeasurementService > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.gms.messaging.cpp.MessageForwardingService > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.firebase.components.ComponentDiscoveryService > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.firebase.messaging.cpp.RegistrationIntentService > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.firebase.messaging.MessageForwardingService > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.firebase.sessions.SessionLifecycleService > /dev/null 2>&1
pm disable com.dts.freefireth/com.unity3d.plugin.downloader.UnityDownloaderService > /dev/null 2>&1
pm disable com.dts.freefireth/com.voxelbusters.androidlib.internal.ScreenRecordingService > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.profileinstaller.ProfileInstallReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.background.systemalarm.ConstraintProxy$BatteryChargingProxy > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.background.systemalarm.ConstraintProxy$BatteryNotLowProxy > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.background.systemalarm.ConstraintProxy$NetworkStateProxy > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.background.systemalarm.ConstraintProxy$StorageNotLowProxy > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.background.systemalarm.ConstraintProxyUpdateReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.diagnostics.DiagnosticsReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.work.impl.utils.ForceStopRunnable$BroadcastReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/com.appsflyer.MultipleInstallBroadcastReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/com.dts.freefireth.FFBroadcastReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/com.facebook.AuthenticationTokenManager$CurrentAuthenticationTokenChangedBroadcastReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/com.facebook.CurrentAccessTokenExpirationBroadcastReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.datatransport.runtime.scheduling.jobscheduling.AlarmManagerSchedulerBroadcastReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.android.gms.measurement.AppMeasurementReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/com.google.firebase.iid.FirebaseInstanceIdReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/com.unity3d.plugin.downloader.UnityAlarmReceiver > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.core.content.FileProvider > /dev/null 2>&1
pm disable com.dts.freefireth/androidx.startup.InitializationProvider > /dev/null 2>&1
pm disable com.dts.freefireth/com.facebook.FacebookContentProvider > /dev/null 2>&1
pm disable com.dts.freefireth/com.facebook.internal.FacebookInitProvider > /dev/null 2>&1
...
rm -f -r /data/user/0/com.dts.freefireth/shared_prefs/.xml > /dev/null 2>&1
